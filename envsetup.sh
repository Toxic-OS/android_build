...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^toxic_") ; then
       ...
       export BUILD_NUMBER=$((date +%s%N ; echo $TOXIC_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       TOXIC_BUILD=
    fi
    export TOXIC_BUILD
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/toxic/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var TOXIC_VERSION)
        ZIPFILE=ROM-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.toxic.device | grep -q "$TOXIC_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $TOXIC_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.toxic.device | grep -q "$TOXIC_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $TOXIC_FIXUP_COMMON_OUT ]; then
